...
def checkParking(imgProcess):
    ...
    for pos in posList:
        ...
        cvzone.putTextRect(img, str(count), (x, y + height - 3), scale=1, thickness=2, offset=0,
                           colorR=color)
    cvzone.putTextRect(img, f'Free Spaces:{spaceCounter}/{len(posList)}'
                       , (100, 45), scale=3, thickness=5, offset=15,
                       colorR=(0, 200, 0))
...
while True:
    ...
    #cv2.imshow('ImageTHre', imgMedian)
    cv2.waitKey(1)